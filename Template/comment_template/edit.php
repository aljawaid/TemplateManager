<div class="modal-page-header template-manager-modal-header">
    <h2 class="relative">
        ...
        <span class="modal-title">
            ...
            </svg> <?= t('Edit Task Comment Template') ?> <span class="modal-template-id">#<?= $template['id'] ?></span>
        </span>
    </h2>
</div>
<div class="modal-contents template-manager-modal-contents">
    <form id="TemplateForm" class="template-form" method="post" action="<?= $this->url->href('CommentTemplateController', 'update', array('project_id' => $project['id'], 'id' => $template['id'], 'plugin' => 'TemplateManager')) ?>" autocomplete="on">
        <?= $this->form->csrf() ?>
